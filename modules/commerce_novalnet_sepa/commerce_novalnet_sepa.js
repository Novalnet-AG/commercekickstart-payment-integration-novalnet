...
(function ($) {
    Drupal.behaviors.commerceNovalnetSEPA = {
      attach: function (context, settings) {
        if ($('input#nn_sepa_hash').size() > 0) {
          ...
          if ($('input#nn_sepa_refill_hash').val() != '' &&  $('input#nn_sepa_refill_inprocess').val() != 1) {
            sepa_refil()
          ...
          $(form).submit(function (event) {
            ...
            if ($('input#nn_sepa_hash').size() > 0) {
              ...
              if (!$('.nnsepa_ibanconf').is(':checked')) {
                ...
                $("input[type=submit]:disabled", form).remove();
                return false;
              }
            }
            return true;
  ...
  function show_sepa_mask(form) {
    ...
    $('#nnsepa_normal_form', form).hide();
    $('.nn_fraud_data', form).hide();
  ...
  function hide_sepa_mask(form) {
    ...
    $('#nnsepa_normal_form', form).show();
    $('.nn_fraud_data', form).show();
  }
  function reset_sepa_fields(form) {
    $(form).removeClass('novalnet-sepa-processed');
    $('#nnsepa_acno', form).val('');
    $('#nnsepa_bcode', form).val('');
    $('#nn_sepa_hash', form).val('');
  ...
  function novalnet_get_sepa_hash(form) {
    ...
    var sepaAcno = $.trim($('#nnsepa_acno').val());
    var sepaBcode = $.trim($('#nnsepa_bcode').val());
    var nnAuthcode = $('#nn_auth_code').val();
    var nnUniqueid = $('#nn_unique_id').val();
    var nnVendor = $('#nn_vendor_id').val();
    sepaBbic = sepaBcode;
    ...
    ibanReq = {'account_holder':  sepaName , 'vendor_id' : nnVendor , 'vendor_authcode' : nnAuthcode , 'bank_country' : sepaCountry , 'unique_id' : nnUniqueid };
  ...
  function sepa_refil() {
    ...
    var nnSepaHash = $('#nn_sepa_refill_hash').val();
    var nnAuthcode = $('#nn_auth_code').val();
    var nnUniqueid = $('#nn_unique_id').val();
    var nnVendor = $('#nn_vendor_id').val();
    var qryString = {"vendor_id" : nnVendor, "vendor_authcode" : nnAuthcode, "unique_id":nnUniqueid, "sepa_data_approved":1, "mandate_data_req" :1, "sepa_hash" :nnSepaHash}
